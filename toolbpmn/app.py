...
if _has_name:
    ...
    if not st.session_state.get("draft_resolved") and not st.session_state.get("draft_pending"):
        ...
        if _load_res is None:
            # Primer ciclo del componente: un rerun breve (no bloquear con st.stop)
...
if "main_process_input" not in st.session_state:
    st.session_state.main_process_input = ""
...
with tab_txt:
    st.text_area(
        ...
        key="main_process_input",
    )
    if _has_name:
        input_text = st.session_state.get("main_process_input", "")
...
if _has_name and input_text.strip() and not st.session_state.get("process_data"):
    try:
        ...
        _txt_hash = draft_content_hash(_payload_txt)
        if st.session_state.get("_draft_saved_hash") != _txt_hash:
            draft_save(
                st.session_state["user_name"],
                _payload_txt,
                key=f"dsave_txt_{st.session_state['user_name']}",
            )
            st.session_state._draft_saved_hash = _txt_hash
    except Exception:
        pass
...
if st.session_state.process_data and not _has_name:
    ...
    try:
        from draft_store import draft_save, build_draft_payload, draft_content_hash
        _payload = build_draft_payload(data, st.session_state.get("bpmn_xml"))
...
<div style="display:flex;gap:11px;margin-bottom:14px;flex-wrap:wrap">
  ...
  <div style="flex:1;min-width:100px;background:white;border-radius:10px;
    ...
    if _fp_now != st.session_state.get("_data_fp"):
        if st.session_state.get("_sync_source") != "diagram":
            try:
                ...
                st.session_state._sync_banner = f"No se pudo sincronizar el diagrama: {e}"
        else:
    ...
    # Asegurar XML/PNG aunque solo exista process_data
    if st.session_state.process_data and (
        not st.session_state.bpmn_xml or not st.session_state.diagram_png
    ):
        try:
            _sync_tables_to_diagram()
        except Exception as e:
            st.warning(f"No se pudo generar el diagrama automaticamente: {e}")
            with st.expander("Detalle del error de diagrama"):
                st.code(traceback.format_exc())
...
# ── Footer ────────────────────────────────────────────────────────────────────
_f1, _f2 = st.columns([1, 4])
with _f1:
    st.link_button("Recargar pagina", url=".", use_container_width=True, help="Util si la app despertó lenta o no responde")
with _f2:
    st.markdown(
