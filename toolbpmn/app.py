            ...
            try:
                ...
                for chars_done, chars_total, result in analyze_process_stream(
                    ...
                    if result is not None:
                        ...
                        bar.progress(pct, text=f"Claude generando… {chars_done} caracteres")

                bar.progress(100, text="Listo.")
                errors = validate_process_json(data)
                if errors:
                    status.update(label="Advertencias en el análisis", state="error", expanded=True)
                    ...
                    status.update(
                        label=f"✅ '{data.get('nombre_proceso')}' — {n} pasos",
                        state="complete", expanded=False)
            except Exception as e:
                status.update(label="Error", state="error")
                st.error(str(e))
    else:
        # ── Modo simple (fallback sin streaming) ──────────────────────────────
        with st.spinner("Claude está analizando el proceso…"):
            try:
