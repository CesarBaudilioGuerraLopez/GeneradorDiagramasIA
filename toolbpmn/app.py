...
from analyzer import analyze_process, analyze_process_stream, validate_process_json
...
with file_tab:
    ...
    if template_path.exists():
        ...
        with col_hint:
            st.info("Usa la plantilla para que la herramienta identifique automáticamente "
                    "roles, responsables y tiempos sin ambigüedad.")
    ...
    if uploaded_file:
        try:
            ...
            # ── Mostrar columnas detectadas si es Excel ──────────────────────
            if meta.get("modo") == "plantilla":
                cols_det = meta.get("columnas_detectadas", {})
                filas = meta.get("filas_detectadas", 0)
                st.success(
                    f"**{uploaded_file.name}** — Plantilla detectada con {filas} pasos"
                )
                if cols_det:
                    _FIELD_LABELS = {
                        "id": "ID del paso", "nombre": "Nombre actividad",
                        "tipo": "Tipo", "rol": "Responsable / Área",
                        "descripcion": "Descripción", "tiempo_ejecucion": "Tiempo",
                        "unidad_tiempo": "Unidad", "documentacion": "Instrucciones",
                        "condicion": "Condición decisión", "siguiente": "Siguiente paso",
                    }
                    ...
                    badge_html = " ".join(
                        ...
                        f'✅ <b>{_FIELD_LABELS.get(campo, campo)}</b> '
                        f'← <code>{orig}</code></span>'
                    ...
                    if missing:
                        ...
                        st.warning(f"No se encontraron: **{', '.join(labels_miss)}**. "
                                   "Claude intentará inferirlas del contexto.")

            elif meta.get("modo") == "generico" and uploaded_file.name.endswith((".xlsx", ".xls")):
                st.warning(
                    "No se reconocieron encabezados estándar en el Excel. "
                    "Se enviará como texto plano. Para mejor reconocimiento, "
                    "descarga y usa la **plantilla CASSA**."
                )
                st.success(f"**{uploaded_file.name}** — {len(extracted):,} caracteres extraídos")
            else:
                st.success(
                    f"**{uploaded_file.name}** leído correctamente — {len(extracted):,} caracteres"
                )

            with st.expander("Ver texto que se enviará a Claude"):
...
if analyze_btn and input_text.strip() and api_key:
    with st.status("Analizando proceso con Claude...", expanded=True) as status:
        try:
            ...
            progress_bar = st.progress(0, text="Generando estructura del proceso…")
            info_slot    = st.empty()
            data         = None
            ...
            for chars_done, chars_total, result in analyze_process_stream(
                ...
                if result is not None:
                    ...
                    progress_bar.progress(
                        pct,
                        text=f"Claude generando… {chars_done} caracteres recibidos"
                    )

            progress_bar.progress(100, text="Procesando respuesta…")
            info_slot.empty()

            errors = validate_process_json(data)
            if errors:
                status.update(label="Análisis con advertencias", state="error", expanded=True)
                ...
                status.update(
                    label=f"✅ '{data.get('nombre_proceso')}' — {n_roles} roles, {n_pasos} pasos",
                    state="complete",
                    expanded=False,
                )
