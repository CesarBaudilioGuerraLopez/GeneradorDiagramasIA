"""Renderiza el proceso como imagen PNG usando matplotlib — layout adaptativo."""
...
# ── Dimensiones base (pulgadas) ───────────────────────────────────────────────
TITLE_H  = 0.5    # alto del título
LANE_H   = 2.2    # alto fijo de cada swim lane (no escala)
LABEL_W  = 1.1    # ancho de etiqueta lateral del lane
X_PAD    = 0.5    # margen izquierdo dentro del lane
R_PAD    = 0.6    # margen derecho

# Dimensiones de nodo en proporción al paso de columna
_NODE_W  = 1.9    # ancho tarea
_NODE_H  = 0.65   # alto tarea
_H_GAP   = 0.7    # espacio entre columnas

# Límite de ancho de imagen destino (pulgadas a 140 DPI → 2800 px)
MAX_FIG_W = 20.0
TARGET_DPI = 140
...
def render_diagram(data: dict[str, Any]) -> bytes:
    ...
    # ── Asignación de columnas (BFS + resolución de conflictos) ──────────────
    col_map = _assign_columns(pasos, role_idx)
    max_col = max(col_map.values(), default=0)

    # ── Escala x: si hay muchos pasos limitamos el ancho total ───────────────
    nat_w   = LABEL_W + X_PAD + (max_col + 1) * (_NODE_W + _H_GAP) + R_PAD
    if nat_w > MAX_FIG_W:
        x_scale = (MAX_FIG_W - LABEL_W - X_PAD - R_PAD) / ((max_col + 1) * (_NODE_W + _H_GAP))
    else:
        x_scale = 1.0

    NODE_W  = _NODE_W  * x_scale
    H_GAP   = _H_GAP   * x_scale
    NODE_H  = max(0.4, _NODE_H * max(0.6, x_scale))   # escala ligera en alto
    FSIZE   = max(5.5, 7.5 * x_scale)                 # fuente proporcional

    fig_w = min(nat_w, MAX_FIG_W)
    ...
    ax.text(fig_w / 2, fig_h - TITLE_H / 2, title,
            ha="center", va="center", fontsize=min(13, 13 * x_scale + 6),
            fontweight="bold", color="#1a2332")

    # ── Swim lanes ────────────────────────────────────────────────────────────
    for i, role in enumerate(roles):
        color  = LANE_BG[i % len(LANE_BG)]
        lane_y = fig_h - TITLE_H - (i + 1) * LANE_H
    ...
    for paso in pasos:
        ...
        label = _wrap(paso.get("nombre", pid), max(10, int(18 * x_scale)))

        if tipo in ("inicio", "fin"):
            r = max(0.18, 0.32 * x_scale)
            ax.add_patch(plt.Circle((cx, cy), r, color=fill, zorder=3))
            if tipo == "fin":
                ax.add_patch(plt.Circle((cx, cy), r * 0.65, color="white", zorder=4))
            ax.text(cx, cy, label, ha="center", va="center",
                    fontsize=FSIZE * 0.9, fontweight="bold",
                    color=fill if tipo == "fin" else fg, zorder=5)

        elif tipo == "decision":
            s = max(0.22, 0.42 * x_scale)
            ...
            ax.text(cx, cy, label, ha="center", va="center",
                    fontsize=FSIZE * 0.85, fontweight="bold", color=fg,
            ...
            ax.text(cx, cy, label, ha="center", va="center",
                    fontsize=FSIZE, fontweight="bold", color=fg,
    ...
    for paso in pasos:
        ...
        for i, nxt in enumerate(paso.get("siguiente", [])):
            ...
            # Radio efectivo por tipo
            sr = _node_rx(tipo, x_scale, NODE_W)
            tr = _node_rx(tgt_tipo, x_scale, NODE_W)

            sx, sy = scx + sr, scy
            ex, ey = tcx - tr, tcy

            if abs(scy - tcy) > 0.05:   # cruce de lane
                mid_x = (sx + ex) / 2
            ...
            if tipo == "decision":
                ...
                ax.text(mx, my + 0.08, lbl, fontsize=FSIZE * 0.9,
                        color="#922b21", ha="center", va="bottom", fontweight="bold",
                        bbox=dict(boxstyle="round,pad=0.1", fc="white", ec="none", alpha=0.8))

    plt.tight_layout(pad=0.3)
    buf = io.BytesIO()
    plt.savefig(buf, format="png", dpi=TARGET_DPI, bbox_inches="tight",
...
def _node_rx(tipo: str, x_scale: float, node_w: float) -> float:
    """Radio / semi-ancho del borde derecho del nodo."""
    if tipo in ("inicio", "fin"):
        return max(0.18, 0.32 * x_scale)
    if tipo == "decision":
        return max(0.22, 0.42 * x_scale)
    return node_w / 2


def _assign_columns(pasos: list, role_idx: dict) -> dict[str, int]:
    ...
    # Resolver conflictos (mismo col + mismo lane)
    ...
    # Propagar: nodo destino siempre en col > origen
    for _ in range(len(pasos) * 2):
        moved = False
        for paso in pasos:
            for nxt in paso.get("siguiente", []):
